# Protect the Huzzi AI n8n workflow
...
2. Change the Webhook path from `meal-ai` to a new path, such as `huzzi-meal-analysis-v1`.
3. Set **HTTP Method** to `POST` and keep the existing binary file field named `file`.
4. Set **Authentication** to `Header Auth`.
5. Create an n8n Header Auth credential named `Huzzi Edge Gateway`:
   - Header name: `X-Huzzi-Webhook-Secret`
...
- A POST without `X-Huzzi-Webhook-Secret` must be rejected before any AI node executes.
